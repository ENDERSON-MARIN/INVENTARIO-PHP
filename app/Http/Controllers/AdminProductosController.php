<?php namespace App\Http\Controllers;
	...
	class AdminProductosController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Referencia','name'=>'referencia','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			//$this->form[] = ["label"=>"Referencia","name"=>"referencia","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
        ...
        public function getAdd (){
			$data['page_title'] = "Registrar Nuevo Producto";

			$date = Carbon::now();
			$fecha_hora = $date->format('d-m-Y H:i:s a');
			$responsable_creacion = CRUDBooster::myId();
			//$privilegio_id = CRUDBooster::myPrivilegeId();
            ...
            //dd($fecha_hora, $responsable_creacion, $categorias,$stock_inicial, $estado_producto);

			$this->cbView('productos.crear', compact('data', 'fecha_hora', 'responsable_creacion', 'categorias', 'stock_inicial', 'estado_producto'));
		...
		public function store(ProductoFormRequest $request)
		{
			try {
				...
				$producto->categoria_id=$request->input('slCategoria');
				$producto->nombre=$request->input('txtNombre');
				$producto->referencia=$request->input('txtReferencia');
				$producto->peso=$request->input('txtPeso');
				$producto->stock=$request->input('txtStock');
				$producto->estado=$request->input('slEstado');
				$producto->fecha_elaboracion=$request->input('txtFechaElaboracion');
				$producto->observaciones=$request->input('txtObservaciones');
				$producto->responsable_creacion=$request->input('txtResponsableCreacion');
				...
				Alert::error('ERROR AL AGREGAR EL PRODUCTO, POR FAVOR VERIFIQUE!', '');
		...
		public function getEdit ($id){
			...
			//dd($productos, $categorias, $fecha_hora, $responsable_edicion, $stock_inicial, $estado_producto);

			$this->cbView('productos.edit', compact('data', 'producto', 'categorias', 'fecha_hora', 'responsable_edicion', 'stock_inicial', 'estado_producto'));

		}


		public function postProductosEdit(Request $request, $id)
			{
                $producto = Producto::findOrFail($id);
				...
				try {
                    ...
                    $producto->fecha_elaboracion=$request->input('txtFechaElaboracion');
                    $producto->observaciones=$request->input('txtObservaciones');
                    $producto->responsable_edicion=$request->input('txtResponsableEdicion');
					...
					Alert::error('ERROR AL EDITAR EL PRODUCTO, POR FAVOR VERIFIQUE!', '');
 		...
 		public function getDetail ($id){
			$data['page_title'] = "Ver Detalles del Producto";

			$producto = DB::table('productos')
			->join('cms_users', 'productos.responsable_creacion', '=', 'cms_users.id')
			->join('categorias', 'productos.categoria_id', '=', 'categorias.id')
			->select('productos.*', 'categorias.nombre as categoria', 'cms_users.name as creado_por')
			->where('productos.id', $id)
			->get();

			//dd($producto);

			$this->cbView('productos.show', compact('id', 'data', 'producto'));
